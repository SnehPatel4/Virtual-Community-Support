...
export class AddMissionComponent implements OnInit {
  ...
  get cityId() {
    return this.addMissionForm.get('cityId') as FormControl;
  }
  get missionTitle() {
    return this.addMissionForm.get('missionTitle') as FormControl;
  }
  get missionDescription() {
    return this.addMissionForm.get('missionDescription') as FormControl;
  }
  get startDate() {
  ...
  get missionThemeId() {
    return this.addMissionForm.get('missionThemeId') as FormControl;
  }
  get missionSkillId() {
    return this.addMissionForm.get('missionSkillId') as FormControl;
  }
  get missionImages() {
    return this.addMissionForm.get('missionImages') as FormControl;
  }
  get totalSheets() {
    return this.addMissionForm.get('totalSheets') as FormControl;
  }
  ...
  async OnSubmit() {
    ...
    value.missionSkillId = Array.isArray(value.missionSkillId)
      ? value.missionSkillId.join(',')
      : value.missionSkillId;
    value.missionThemeId = Array.isArray(value.missionThemeId)
      ? value.missionThemeId.join(',')
    ...
    if (this.addMissionForm.valid) {
      ...
      console.log(payload);
      this.service.AddMission(payload).subscribe(
        (data: any) => {
          if (data.result == 1) {
            this.toast.success({
              detail: 'SUCCESS',
        ...
        (err) => {
          ...
          this.toast.error({
            detail: 'ERROR',
            summary: 'Some other error occurred',
            duration: 3000,
          });
        }
      );
